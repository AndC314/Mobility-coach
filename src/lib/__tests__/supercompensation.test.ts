...
describe('computeSupercompensation', () => {
  ...
  it('shows dip below 100 on day+1 after a hard push session', () => {
    const log = makeCalLog({ exerciseId: 'pushups', date: daysAgo(3), value: 20, sets: 4 })
  ...
  it('recovers above baseline by day+5 after a session', () => {
    const log = makeCalLog({ exerciseId: 'pushups', date: daysAgo(10), value: 20, sets: 4 })
    const result = computeSupercompensation([log], [], 30)
  ...
  it('uses different time constants per category', () => {
    const pushLog = makeCalLog({ exerciseId: 'pushups', date: daysAgo(5), value: 20, sets: 4 })
  ...
  it('minimum impulse floor: light session still produces adaptation', () => {
    const lightLog = makeCalLog({ exerciseId: 'pushups', date: daysAgo(5), value: 1, sets: 1 })
  ...
  it('atrophy kicks in after 10 inactive days', () => {
    const log = makeCalLog({ exerciseId: 'pushups', date: daysAgo(25), value: 20, sets: 4 })
    const result = computeSupercompensation([log], [], 30)
    const today = result[result.length - 1]
  ...
  it('atrophy floor at 70', () => {
    const log = makeCalLog({ exerciseId: 'pushups', date: daysAgo(80), value: 20, sets: 4 })
  ...
  it('two sessions produce higher fitness than one', () => {
    const oneSession = [makeCalLog({ exerciseId: 'pushups', date: daysAgo(7), value: 20, sets: 4 })]
    const twoSessions = [
      makeCalLog({ exerciseId: 'pushups', date: daysAgo(10), value: 20, sets: 4 }),
      makeCalLog({ exerciseId: 'pushups', date: daysAgo(5), value: 20, sets: 4 }),
    ]
  ...
  it('density factor: shorter rest produces bigger impulse', () => {
    const normalRest = makeCalLog({ exerciseId: 'pushups', date: daysAgo(5), value: 10, sets: 3, restSec: 90 })
    const shortRest = makeCalLog({ exerciseId: 'pushups', date: daysAgo(5), value: 10, sets: 3, restSec: 30 })
